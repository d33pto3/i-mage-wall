...
import { X, Upload, Image as ImageIcon, CheckCircle2, Loader2, AlertCircle } from "lucide-react";
  ...
  React.useEffect(() => {
    if (isVisible && user) {
      const checkLimit = async () => {
        ...
        try {
          const q = query(collection(db, "pictures"), where("userId", "==", user.uid));
          ...
          >
            <div className="p-8">
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h2 className="text-2xl font-bold text-[var(--text-primary)]">Add to Wall</h2>
                  <p className="text-sm text-[var(--text-secondary)]">Drop your creative assets here.</p>
                </div>
                <button onClick={closeModal} className="p-2 hover:bg-[var(--bg-secondary)] rounded-full transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              {!preview ? (
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="font-bold text-[var(--text-primary)]">Select Image</p>
                ...
                <div className="space-y-6">
                  {imageCount >= 3 && (
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        <p className="text-sm font-bold text-red-500">Global Limit Reached</p>
                        <p className="text-xs text-red-500/70">The Free Tier is restricted to 3 active assets. Please remove an existing image to inject a new one.</p>
                      </div>
                    </motion.div>
                  )}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-secondary)] ml-1">Asset Designation</label>
                    <input 
                      type="text" 
                      value={picTitle}
                  ...
                  <button
                    ...
                    disabled={uploading || status === 'success' || imageCount >= 3 || isCheckingLimit}
