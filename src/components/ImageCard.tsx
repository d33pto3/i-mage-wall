...
import { Bookmark, Download, Trash2, Eye, EyeOff, Check, Image as ImageIcon, Plus } from 'lucide-react';
...
const ImageCard: React.FC<ImageCardProps> = ({
  ...
  return (
    <motion.div 
      className={`group relative rounded-xl overflow-hidden cursor-zoom-in transition-all duration-700 bg-[var(--bg-secondary)] ${
    ...
    >
      ...
      {canEdit && (
        <button 
          onClick={(e) => { e.stopPropagation(); onSelect?.(picture.id); }}
      ...
      {/* Primary Actions Overlay (Top Right) */}
      <div className="absolute top-4 right-4 z-30 flex gap-2 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-500">
        <button 
          ...
          className={`w-10 h-10 rounded-lg flex items-center justify-center backdrop-blur-md border border-white/20 transition-all ${isSaved ? 'bg-white text-[var(--accent-color)]' : 'bg-black/20 text-white hover:bg-white/40'}`}
        >
      ...
      <div className="relative overflow-hidden">
        {imgError ? (
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center text-[var(--text-secondary)]">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">Architecture Void</p>
          </div>
        ) : (
          <motion.img
            src={picture?.url}
        ...
        <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/60 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-10" />
      </div>

      {/* Footer Details (Bottom Left + Right) */}
      <div className="absolute inset-x-0 bottom-0 z-30 p-5 flex items-center justify-between opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-500">
        ...
        <div className="flex items-center gap-2">
          {canEdit && (
            <button 
              ...
              className="p-2 rounded-lg bg-black/20 backdrop-blur-md border border-white/10 text-white hover:bg-white/20 transition-all"
            >
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white/90 hover:bg-white text-black rounded-lg text-[10px] font-bold transition-all shadow-xl active:scale-95 group/btn"
          >
      ...
      {canEdit && !isSelected && (
        <div className="absolute top-4 left-14 z-20">
          <div className={`px-2 py-1 rounded-md backdrop-blur-md border border-white/20 text-[8px] font-black uppercase tracking-widest ${
            picture.isPublic ? 'bg-green-500/20 text-green-300' : 'bg-amber-500/20 text-amber-300'
          }`}>
