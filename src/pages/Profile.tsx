...
function Profile() {
  ...
  const handleBatchDelete = async () => {
    ...
    if (confirm(`Delete ${selectedIds.length} assets?`)) {
      try {
        await Promise.all(
          selectedIds.map((id) => deleteDoc(doc(db, "pictures", id)))
        );
        setSelectedIds([]);
      } catch (err) {
  ...
  const handleBatchPrivacy = async (makePublic: boolean) => {
    ...
    try {
      await Promise.all(
        selectedIds.map((id) =>
          updateDoc(doc(db, "pictures", id), { isPublic: makePublic })
        )
      );
      setSelectedIds([]);
    } catch (err) {
  ...
  const stats = [
    {
      label: "Storage Slots",
      value: `${storageAssetsCount} / 3`,
      icon: Layout,
      progress: (storageAssetsCount / 3) * 100,
    },
    {
      label: "Remote Links",
      value: remoteAssetsCount === 0 ? "Void" : `${remoteAssetsCount} Loaded`,
      icon: Compass,
    },
      ...
      <div className="max-w-[1700px] mx-auto px-6 md:px-12 flex flex-col lg:flex-row gap-8 pb-20">
        ...
        <aside className="w-full lg:w-80 flex flex-col gap-6">
          <div className="glass rounded-[2.5rem] p-10 flex flex-col items-center text-center shadow-sm border-white/10">
            ...
            <div className="mb-10">
              <h2 className="text-2xl font-black text-[var(--text-primary)] mb-1 tracking-tight">
                {user?.displayName}
              </h2>
          ...
          <div className="glass rounded-[2rem] p-8 shadow-sm border-white/10">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--text-secondary)] mb-8 px-2 opacity-50">
              Curated Collections
            </h3>
            <div className="space-y-2">
              {albums.map((album, i) => (
                <button
                  key={i}
                  className="w-full flex items-center justify-between p-4 rounded-2xl hover:bg-[var(--bg-primary)] transition-all group border border-transparent hover:border-white/20"
                >
        ...
        <main className="flex-1 bg-[var(--bg-primary)] rounded-[3rem] p-4 md:p-10 shadow-xl border border-[var(--border-color)] overflow-hidden">
          <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between mb-12 gap-8 px-2">
            <div className="space-y-3">
              ...
              <p className="text-base text-[var(--text-secondary)] font-medium max-w-lg">
                Manage, orchestrate, and deploy your visual assets across the
                global architecture.
              </p>
            </div>
              ...
              <div className="flex bg-[var(--bg-secondary)] p-1.5 rounded-[1.5rem] border border-[var(--border-color)]">
                {[
                  ...
                  <button
                    ...
                    className={`px-8 py-3 rounded-[1.2rem] text-[10px] font-black uppercase tracking-[0.2em] transition-all ${
                      activeTab === tab.id
          ...
          <div className="mb-12 glass rounded-[2rem] p-6 border-dashed border-2 border-[var(--accent-color)]/20">
            <div className="flex flex-col md:flex-row items-center gap-4">
              <div className="flex-1 w-full space-y-1">
                <label className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--accent-color)] ml-1">
                  Remote URL (Unlimited)
                </label>
          ...
          <div className="relative min-h-[60vh]">
            {filteredList.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-48 border-2 border-dashed border-[var(--border-color)] rounded-[3rem] bg-[var(--bg-secondary)]/30 mx-2">
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-10 font-medium">
                  No visual assets detected in this sector.
                </p>
                <button
                  onClick={() => setIsUploadOpen(true)}
      ...
      >
        <div className="glass rounded-[2rem] p-4 flex items-center gap-6 shadow-2xl border-white/20 bg-black/90 dark:bg-white/90">
          ...
          <div className="flex gap-2 pr-2">
            ...
            >
              <Trash2 size={20} />
              <span className="text-[10px] font-black uppercase tracking-widest hidden md:block text-red-500/80">
                Destroy
              </span>
            </button>
          </div>
